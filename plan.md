...
- *Models to define:*
  ...
  - **`AuditEvent`** (any → audit channel): `event_id`, `kind ∈ {"message_sent","lt_verdict","constraint_violation","bypass_triggered","rho_debited","budget_exhausted","human_review","rate_limit"}`, `actor_agent_id`, `payload: AuditPayload` (typed by `kind`), `prev_event_hash`, `event_hash`.
...
- *Rule constraints (LLM cannot override) — implemented as `ConstraintRule` objects in `backend/agents/a1_monitoring.py`:*
  ...
  3. **`bypass_candidates_must_emit`** — any candidate matching `A1-B1`, `A1-B2`, or `A1-B3` MUST result in `action="emit"` with a populated `alert`, the expected `bypass_rule_id`, and the deterministic signal type and severity for that bypass. The LLM cannot suppress or remap a bypass-matching candidate.
...
- *Acceptance (current):* `uv run pytest tests/test_a1.py` passes 11 focused P6 tests (8 originally + 1 end-to-end S1 LLM-path test + 2 review regressions for mixed-batch bypass and evidence leakage). `uv run python -m backend.agents.a1_monitoring demo --bank bank_alpha --limit 10 --stub` runs and prints the local A1 table with LLM-judgment rows, an S1 local row, and A1-B1/A1-B2 bypass rows. `uv run pytest` passes the full 89-test suite. `uv run python -m compileall backend shared tests` passes. `git diff --check` reports only CRLF normalization warnings.
...
- *Budget ledger:*
  ...
  - On exhaustion: every primitive call from that requester returns `(None, rec_with_refusal_reason="budget_exhausted")`. The refusal is structural; A3 cannot retry around it.
