...
- *Models to define:*
  ...
  - **`Sec314bQuery`** (A2 → F1 → peer A3): `query_id`, `requesting_investigator_id`, `requesting_bank_id`, `target_bank_ids` (default: all peers), `query_shape ∈ {"entity_presence","aggregate_activity","counterparty_linkage"}` (discriminated union), `query_payload` (typed by `query_shape`; entity/counterparty identifiers are `CrossBankHashToken`), `purpose_declaration: PurposeDeclaration`, `requested_rho_per_primitive: float`. `PurposeDeclaration` is a structured object: `{authority: "USA_PATRIOT_314b", typology_code: TypologyCode, suspicion_rationale: str (≤500 chars), supporting_alert_ids: list[UUID]}`.
...
- *Rule constraints (LLM cannot override) — implemented as `ConstraintRule` objects in `backend/agents/a1_monitoring.py`:*
  ...
  5. **`evidence_uses_hashed_identifiers`** — `Alert.evidence[*]` carries the expected entity hash plus SHA-256 hashes of the source account and transaction IDs. Raw account or transaction IDs must not appear in any evidence field, including `summary`, `entity_hashes`, `account_hashes`, or `transaction_hashes`.
...
- *Budget ledger:*
  ...
  - Each Gaussian-mechanism call debits its declared ρ only after the primitive's SQLite read, OpenDP calibration, and noise construction succeed. The ledger sums ρ values and refuses when cumulative ρ would exceed `rho_max`. Do not mix the approximate-DP σ formula with this ledger; the zCDP Gaussian calibration is σ = sensitivity / sqrt(2ρ).
...
- *What was built:* `backend/silos/budget.py` defines `RequesterKey`, `BudgetDebit`, `BudgetSnapshot`, and `PrivacyBudgetLedger` keyed by requesting investigator, requesting bank, and responding bank. `backend/silos/dp.py` implements zCDP Gaussian sigma math, approximate `(ε, δ)` display conversion, OpenDP privacy-map validation, and deterministic RNG injection. `backend/silos/stats_primitives.py` implements the five bank-local primitives over SQLite, `DateWindow`, and `PrimitiveResult`. DP primitives check budget before work, debit budget only after SQLite reads, OpenDP calibration, and noise construction succeed, return structural refusals on exhaustion, and emit provenance records with args hashes, sensitivity, rho, sigma, and returned value kinds.
- *Acceptance (current):* `uv run pytest tests/test_budget.py tests/test_dp_composition.py tests/test_stats_primitives.py` passes 14 focused P7 tests. These cover rho composition and budget refusal, budget-key isolation and snapshot round-trip, OpenDP Gaussian map agreement, empirical Gaussian mean/variance with seeded RNG, exact non-DP entity and counterparty primitives, DP metadata on alert counts and histograms, F2 bank aggregate shape, structural budget exhaustion refusal, and no budget debit when a DP primitive fails before returning a result or when noise sampling fails. `uv run pytest` passes the full 106-test suite.
...
- *Acceptance:* `tests/test_a2.py`:
  ...
  - **Test 2 (response synthesis):** feed A2 an aggregated `Sec314bResponse` with corroborating peer evidence; A2 emits a `SARContribution` that references the response id.
