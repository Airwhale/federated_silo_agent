...
| F1-B1 | F1 | Missing or empty `PurposeDeclaration.suspicion_rationale` | Reject at schema/signing boundary, with F1 retaining a defense-in-depth refusal path before any LLM call | P9 | `tests/test_f1.py`, `tests/test_messages.py` |
| F1-B2 | F1 | Requester quota exceeded, such as more than 20 queries in 1 hour | Escalate to F5 for compliance review | P14, P15 | `tests/test_aml_policy.py`, `tests/test_orchestrator.py` |
| F1-B3 | F1 | Sanctions-evasion purpose with supplied entity hashes | Route a `SanctionsCheckRequest` through F3 in parallel | P9, P10 | `tests/test_f1.py`, `tests/test_f3.py` |
...
- **Role:** Receives approved peer `Sec314bQuery` messages and same-bank `LocalSiloContributionRequest` messages from F1, maps each request to declared P7 stats primitives, invokes those primitives over local bank data, and returns a `Sec314bResponse` with provenance. A3 is not human-facing and does not make SAR or dismissal decisions.
- **Inputs:** `Sec314bQuery` or `LocalSiloContributionRequest` from verified F1 only.
- **Outputs:** `Sec314bResponse` to F1 only.
- **Reasoning:** Deterministic primitive selection and response-envelope construction, surrounded by signed-envelope, route, purpose, and budget checks. Optional Gemini composition can be enabled only for the primitive-result bundle; P7 computes every value and A3 rejects any composed bundle that changes fields or provenance.
- **Rule constraints (LLM cannot override):**
...
- **Route approval binding:** A3 verifies that the query was approved by F1 for this responding bank and route kind (`peer_314b` or `local_contribution`) before it can invoke P7. F1 approval is necessary, not sufficient.
- **Replay protection:** every envelope has `message_id`, `nonce`, `created_at`, `expires_at`, a five-minute created-at clock-skew check, and an idempotency cache keyed by sender and nonce.
...
A3 verifies F1 signature, F1 route approval, freshness, route kind, purpose, target, query shape, and DP budget. A3 does not independently re-verify the original A2 signature; F1 transitively vouches for A2 after its own inbound-auth checks.
A3 calls P7
...
A3 can refuse with `unsupported_query_shape`, `unsupported_metric`, `unsupported_metric_combination`, `invalid_rho`, `route_violation`, `signature_invalid`, `principal_not_allowed`, `envelope_invalid`, `replay_detected`, `budget_exhausted`, or `provenance_violation`. It never returns raw rows as a negotiation fallback.
...
- *Models to define:*
  ...
  - **`RouteApproval`** (F1 → A3, embedded or referenced by the routed query): `approval_id`, `query_id`, `route_kind ∈ {"peer_314b","local_contribution"}`, `approved_query_body_hash`, `requesting_bank_id`, `responding_bank_id`, `approved_by_agent_id`, `approved_at`, `expires_at`, `signature`, `signing_key_id`. A3 verifies this before invoking P7.
  ...
  - **`GraphPatternRequest`** (F1 → F2): `pattern_aggregates: list[BankAggregate]` (per-bank DP-noised tuples: `bank_id`, `edge_count_distribution`, `bucketed_flow_histogram`, plus approved `candidate_entity_hashes` carried from the originating query for output binding), `window_start`, `window_end`.
...
- *Approach:* Pydantic v2 `BaseModel` with strict mode and `model_config = ConfigDict(extra="forbid")`. Field validators where the constraint is structural: `Sec314bQuery.purpose_declaration.suspicion_rationale` non-empty; `Sec314bResponse.fields.keys()` exactly matches `{p.field_name for p in provenance}`; `SARDraft.mandatory_fields_complete=True` only if all of `filing_institution`, `suspicious_amount_range`, `typology_code`, `narrative` are populated; signed cross-boundary messages require `expires_at`, `nonce`, `body_hash`, `signature`, and `signing_key_id` at the security-envelope layer. Discriminated unions on `query_shape` and `AuditEvent.kind` so Gemini's structured-output can target each variant.
...
- *Acceptance (current):* `uv run pytest tests/test_agent_base.py tests/test_llm_client.py` passes focused P5 tests. `uv run pytest` passes the full suite. `uv run python -m compileall backend shared tests` passes. `git diff --check` reports only CRLF normalization warnings.
...
- *Acceptance (current):* `uv run pytest tests/test_a1.py` passes focused P6 tests, including review regressions for mixed-batch bypass, bypass-policy invariant naming, and evidence leakage. `uv run python -m backend.agents.a1_monitoring demo --bank bank_alpha --limit 10 --stub` runs and prints the local A1 table with LLM-judgment rows, an S1 local row, and A1-B1/A1-B2 bypass rows. `uv run pytest` passes the full suite. `uv run python -m compileall backend shared tests` passes. `git diff --check` reports only CRLF normalization warnings.
...
- *Primitive signatures:*
  ...
  - `flow_histogram(name_hashes: list[str], window, buckets: list[tuple[float,float]], requester, rho: float = 0.03, composition: "parallel_disjoint" | "serial" = "parallel_disjoint") -> (list[int], rec)`: **DP applied per bucket.** Each transaction lands in exactly one bucket after clipping. The demo default is fixed-bucket `parallel_disjoint` accounting: full declared rho is used per bucket, the ledger pays rho total, and provenance records `dp_composition="parallel_disjoint"` plus `per_bucket_rho=rho`. The conservative `serial` mode splits rho across buckets, records `dp_composition="serial"`, and produces noisier histograms for stricter review.
  ...
  - `pattern_aggregate_for_f2(window, requester, candidate_entity_hashes, rho: float = 0.04) -> (BankAggregate, rec)` — **DP applied.** Returns a `BankAggregate` of `edge_count_distribution` (histogram of edge counts on hashed counterparties), `bucketed_flow_histogram`, and the approved `candidate_entity_hashes` supplied by A3/F1. The primitive must not mine unrelated local customer hashes for this field. ρ is split between edge and flow components because they share underlying transactions; within each component, parallel composition uses the full component ρ per bucket. Edge-count sensitivity is L2 `sqrt(2)`, and flow sensitivity is 1.0. This is the input to F2.
...
- *Provenance:* every `PrimitiveCallRecord` carries `field_name`, `primitive_name`, `args_hash` (SHA-256 of canonical JSON of the args, for audit-replay if needed), `privacy_unit`, `rho_debited`, approximate `(ε, δ)` display value, `sigma_applied` (None for non-DP primitives), `sensitivity`, optional `dp_composition` / `per_bucket_rho` for histogram accounting, `returned_value_kind`, `timestamp`. A3 attaches the records to its outbound `Sec314bResponse.provenance`.
- *Histogram display rule:* for histogram records, `rho_debited` and `eps_delta_display` describe the full released histogram, while `per_bucket_rho` and `sigma_applied` describe each bucket's Gaussian mechanism. In `serial` mode, UI panels must show both values so a reviewer sees that total rho composes across buckets even though each bucket was noised with a smaller rho.
...
- *Acceptance:*
  ...
  - **`tests/test_dp_composition.py`** debits a fixed ρ across N primitive calls; verifies the ledger sums via zCDP composition; verifies both fixed-bucket `parallel_disjoint` histogram accounting and conservative serial histogram accounting; the (N+1)th call after budget exhaustion returns the refusal record.
...
- *Acceptance (current):* `uv run pytest tests/test_a2.py` passes focused P8 tests: outbound aggregate query from an S1-style alert, counterparty-linkage query draft construction, query-draft shape validation, corroborating peer response to SAR contribution with `related_query_ids`, empty peer response to dismissal without an LLM call, A2-B1 correlated-alert bypass with within-summary deduplication, future-correlated-alert suppression, inbound peer-query rejection before LLM, A2-B2 sanctions hard escalation, hallucinated name-hash retry/blocking, hallucinated counterparty-hash retry/blocking, response/query id mismatch blocking, F1 response-sender lineage blocking, original-query ownership blocking, supporting-alert lineage blocking, customer-name draft rejection, and malformed triage repair. `uv run pytest tests/test_messages.py tests/test_agent_base.py tests/test_a1.py tests/test_a2.py` passes the focused schema/runtime/A1/A2 set. `uv run pytest` passes the full suite.
...
- *Files:* `backend/agents/a3_silo_responder.py`, `backend/agents/prompts/a3_system.md`, `backend/agents/a3_states.py`, `backend/security/{canonical_json.py,signing.py,principals.py,replay.py}`, `shared/messages.py`, `shared/enums.py`, `infra/demo_principals.yaml` (public allowlist template only), `tests/test_a3.py`, `tests/test_security_envelope.py`, `tests/test_messages.py`.
- *Security sub-slice:*
  ...
  - `PrincipalAllowlist` service that resolves a verified signing key to one `agent_id`/`role`/`bank_id` principal and checks allowed message types, recipients, and routes. Route approvals must be signed by a federation-scoped F1 principal.
...
- *State machine:*
  ...
  4. **`select_primitives`** - deterministic code maps the typed `query_shape` and payload to one or more allowed P7 primitive calls.
  5. **`invoke_primitives`** - deterministic code calls P7, handles budget refusal, and collects `(value, PrimitiveCallRecord)` tuples.
  6. **`compose_response`** - deterministic by default. Optional Gemini composition may echo the primitive bundle, but A3 always wraps the `Sec314bResponse` envelope deterministically. The constraint layer rejects any bundle field that does not exactly match primitive values and provenance.
- *Rule bypasses:* `A3-B1` through `A3-B3` from the Section 8.3 bypass rule catalog are implemented as deterministic refusal paths before primitive or LLM work.
- *Rule constraints (LLM cannot override):*
  ...
  - Cannot spend DP budget when `requested_rho_per_primitive <= 0.0`; DP-backed aggregate metrics fail closed with `refusal_reason="invalid_rho"`.
  - Cannot silently drop metrics or partially spend budget before refusing. A3 accepts one aggregate metric class per request; mixed aggregate metric requests are refused as `unsupported_metric_combination` before any DP primitive runs.
  - Cannot fan out `alert_count` over multiple name hashes because that would multiply the requested budget. Multi-hash activity should use a batched metric such as `flow_histogram` or a future P7 batched alert-count primitive.
  - Cannot accept query options that P7 does not implement. Windowed `entity_presence` and `counterparty_linkage.max_hops > 1` fail closed as `unsupported_query_shape`.
...
- *Approach (Gemini calls):* A3 does not need Gemini for the default demo path. Deterministic code owns primitive selection, value computation, budget checks, and response-envelope construction. Optional `compose_with_llm=True` runs compact primitive-bundle composition through the proxy chain, guarded by exact provenance matching; the LLM never emits `sender_agent_id`, `in_reply_to`, `nonce`, `expires_at`, or signatures.
- *Acceptance (current):* `uv run pytest tests/test_a3.py tests/test_security_envelope.py tests/test_messages.py` passes focused P8a checks:
  ...
  - **Test 8 (provenance guard):** mock Gemini changing primitive values; A3 retries once and then refuses if the response still does not exactly match provenance.
  - **Test 9 (budget semantics):** zero rho for DP-backed metrics refuses before primitives, mixed aggregate metrics refuse before partial execution, and multi-hash `alert_count` refuses before N×rho fan-out.
  - **Test 10 (unsupported query options):** windowed `entity_presence` and multi-hop `counterparty_linkage` refuse before primitive calls until matching P7 primitives exist.
  - **Test 11 (replay state):** replay cache uses tuple keys, not delimiter-joined strings, and snapshots expose nonce hashes plus first-seen/expiry timestamps rather than raw nonces.
- *Acceptance (suite):* `uv run pytest` passes the full suite. Repo-wide `uv run ruff check .` still reports pre-existing lint failures in older data/smoke scripts; `uv run ruff check` over P8a-touched files passes.
...
- *Review notes intentionally deferred:* (M1) raw LLM candidate bodies are not persisted on repair because the current runtime audit stream is metadata-first and avoids storing potentially sensitive model text; add a redacted debug artifact sink later if debugging needs it. (M2) replay cache size is unbounded beyond expiry pruning because the demo volume is small; production should add a max-size or TTL-backed store without evicting live nonces prematurely. (M4) `Sec314bResponse.in_reply_to` for `LocalSiloContributionRequest` uses `source_query_id` by design so F1 can merge requester-bank and peer-bank contributions under one upstream query; the schema docstring documents this convention.
- *Depends on:* P4 (with A3 role added), P5, P7.
- *Scope check:* two focused sessions after P7: first security-envelope/allowlist helpers and schema follow-up, then A3 route handling and primitive invocation.

**P9 — F1 cross-bank coordinator**

- *Goal:* Federation coordinator that receives `Sec314bQuery` from one bank's A2, validates the purpose declaration, routes hash-only queries to relevant peer banks' A3 silo responders, collects responses, and forwards an aggregated `Sec314bResponse` back to the requester. F1 is deterministic over the security-critical data plane; richer LLM or LT purpose scoring can be added later without changing the signed route contract.
- *Current implementation:* deterministic-first F1 is complete for the P9 spine. It verifies signed A2 ingress, signs per-silo route approvals, emits peer A3 requests, emits requester-bank `LocalSiloContributionRequest` for pooled pattern aggregates, emits a sanctions side request for sanctions-evasion typologies, revalidates in-flight original queries without reapplying the ingress clock-skew window, verifies signed A3 responses, binds A3 response nonces to the specific F1 routed request, aggregates namespaced fields/provenance into a signed F1 `Sec314bResponse`, refuses empty aggregation, and converts negotiable silo refusals into at most two revised signed requests. Purpose LLM scoring is intentionally deferred; P9 ships the testable security and routing contract first.
...
- *Acceptance:* `tests/test_f1.py`:
  - Given a valid `Sec314bQuery` from Bank Alpha targeting both peers, F1 emits two hash-only routed queries, one each to Beta and Gamma.
  ...
  - Given a peer target list that includes the requester bank, the P4 schema rejects the query before F1; F1 separately proves it never expands a valid target list.
  - Given a query with empty `PurposeDeclaration.suspicion_rationale`, F1 returns a refusal — LLM is never called.
  - Given a sanctions-evasion purpose with entity hashes, F1 emits a `SanctionsCheckRequest` to F3 in parallel.
  ...
  - Given a negotiable A3 refusal such as `unsupported_metric_combination`, F1 emits a bounded revised signed request with a safe supported metric and a fresh route approval.
  - Given repeated negotiable refusals after the retry limit, F1 stops retrying and returns the silo refusal reason to A2.
- *Acceptance (current):* `uv run pytest tests/test_f1.py` passes 23 F1 tests covering signed route planning, target non-expansion, local contribution routing, blank-purpose signing rejection, tampered-query refusal without signing, principal-denial labeling, sanctions side request for entity and counterparty hashes, signed response aggregation with namespaced provenance, partial-refusal notes, mixed-refusal labeling, deterministic route/aggregate turns without LLM calls, bounded retry planning after a silo refusal, signed route retry-count tracking, retry-limit refusal propagation, aggregation-context revalidation without stale-query clock-skew failure, validated-copy schema enforcement, A3 response nonce binding, route-approval TTL binding for A3 responses, empty aggregation refusal, and near-expiry route handling.
- *Risks specific to this part:* (a) future policy scoring could broadcast more widely than needed ("when in doubt, ask everyone") - mitigation: deterministic constraints cap routing at requester-stated targets. (b) Aggregation logic could lose provenance - mitigation: strict Pydantic validation of the aggregated response.
...
- *Current implementation:* P9b lives in `frontend/` as a Vite React TypeScript app. It uses a committed `openapi-typescript` client schema generated from P9a, TanStack Query polling, hash tabs, a five-column swimlane topology, Radix inspector drawer, interaction console, Demo Flow security probes, a console-embedded system readiness rail, judge-facing About page, and Lobster Trap gate testing. `scripts/start_frontend.ps1` starts the local Vite server on `127.0.0.1:5173`.
...
- *Inputs:* `GraphPatternRequest { pattern_aggregates: list[BankAggregate], window_start, window_end }`. Each `BankAggregate` arrives already DP-noised (computed by the originating bank's P7 `pattern_aggregate_for_f2` primitive); F2 never sees raw transactions. `candidate_entity_hashes` are the approved query tokens carried through for F2 output binding, not top-active local customers mined by P7.
...
- *Judge-safe control API:*
  ...
  - *Aggregate replay policy:* P9 aggregation can re-check signatures and bindings on already-routed artifacts without reapplying original ingress freshness or replay checks. P15 owns the outer one-shot session guard: each routed request and aggregate bundle can be consumed only once per `run_id`/`query_id`, stale bundles from prior runs are rejected at the control/orchestrator layer, and rejected replays emit audit events.
...
- *State snapshot contracts (in `backend/demo/state.py`):*
  ...
  - `RouteApprovalSnapshot`: approval id, query id, route kind, approved body hash, requester/responding bank ids, expiry, signature status, and A3 match/mismatch result.
...
- *Acceptance:*
  ...
  - No final demo panel remains in `not_built` status; any intentionally simulated component is explicitly labeled and justified in the UI.
